...
% SALOA_GROUP_Z_TEST 
% [select_feature, select_group, time] = saloa_group_z_test(group_feature, data, class_attribute, alpha)
...
for i = 1:numgroup
  ...
  for j = 1:length(f_g_index)  
    ...
    [CI, dep(f_g_index(j))] = my_cond_indep_fisher_z(data,f_g_index(j), ...
      class_attribute, [], numInstances, alpha);
    ...
    if ~isempty(current_feature1)
      ...
      for k = 1:p
        [CI, dep_ij] = my_cond_indep_fisher_z(data, f_g_index(j), ...
          current_feature1(k), [], numInstances, alpha);
  ...
  if ~isempty(g{i})
    ...
    for m = 1:i-1
      ...
      for m1 = 1:length(current_feature)
        for m2 = 1:length(g1)
          [CI, dep_ij1] = my_cond_indep_fisher_z(data,g1(m2), current_feature(m1), ...
            [], numInstances, alpha);
